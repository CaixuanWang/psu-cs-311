...
void seek(int did,int bid){
  ...
  uint8_t* block=NULL;
  jbod_client_operation(op,block);
...
int mdadm_mount(void){
  ...
  int result=jbod_client_operation(op,block);
  if(result==0){
    is_mount=1;
    return(1);
  }
...
/* This function tries to find the block is in cache or not.
   If it's in cache, use cache. Otherwise, use jbod_client_operation. */
