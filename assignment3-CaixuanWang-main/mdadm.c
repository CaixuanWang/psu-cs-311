...
void write_check_block(int len,int bid, int did, int offset, uint32_t op, uint32_t op_r,uint8_t buf6[]){
  int block_number=1+(len+offset)/256;
  if(block_number==1){
    uint8_t buf1[256];
    seek(did,bid);
    jbod_operation(op_r,buf1);
    seek(did,bid);
    for(int i=offset;i<offset+len;i++){
      buf1[i]=buf6[i-offset];
    }
    jbod_operation(op,buf1);
  }
  if(block_number==2){
  ...
  if(block_number==3){
    ...
    jbod_operation(op,buf1);
    for(int i=0;i<256;i++){
      buf2[i]=buf6[i-offset+256];
    }
    seek(did,bid+1);
    jbod_operation(op,buf2);
    for(int i=0;i<len-512+offset;i++){
      buf3[i]=buf6[512-offset+i];
    }
    seek(did,bid+2);
    jbod_operation(op,buf3);
  }
  if(block_number==4){
    ...
    jbod_operation(op,buf2);
    for(int i=0;i<256;i++){
      buf3[i]=buf6[i-offset+512];
    }
    seek(did,bid+2);
    jbod_operation(op,buf3);
    for(int i=0;i<len-768+offset;i++){
      buf4[i]=buf6[768-offset+i];
    }
    seek(did,bid+3);
    jbod_operation(op,buf4);
  }
  if(block_number==5){
    ...
    jbod_operation(op,buf3);
    for(int i=0;i<256;i++){
      buf4[i]=buf6[i-offset+768];
    }
    seek(did,bid+3);
    jbod_operation(op,buf4);
    for(int i=0;i<len-1024+offset;i++){
      buf5[i]=buf6[1024-offset+i];
    }
    seek(did,bid+4);
    jbod_operation(op,buf5);
  }
...
int mdadm_read(uint32_t addr, uint32_t len, uint8_t *buf)
{
  ...
  if(mount_time!=1 | len>1024 | len<0| addr+len>JBOD_DISK_SIZE*16){
    return(-1);
  }
  if(len==0 && buf==NULL){
    buf=NULL;
...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf) {
  ...
  if(didafter==did){
    uint8_t buf6[len];
    memcpy(buf6, buf,len);
    write_check_block(len,bid,did,offset,op,op_r,buf6);
    return len;
  }
  else{
    int first_disk_size=(did+1)*JBOD_DISK_SIZE-addr,second_disk_size=len-first_disk_size;
    ...
    write_check_block(first_disk_size,bid,did,offset,op,op_r,buf6);
